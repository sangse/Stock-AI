...
데이터는 상장 후 4개월 간의 데이터를 수집하였다. 신규주가 상장후의 4개월 간의 데이터의 흐름을 중점으로 진행했다. 상장 후 4개월간은 특성상 거래대금이 어느정도 유지가 되고 주가의 흐름 패턴이 존재한다고 생각했기에 그렇게 결정했다.
## 특성과 특성 추가
데이터의 특성으로는 현재가,시가,저가,고가,거래량,거래대금 을 기본으로 데이터를 수집하였고, 이 수집한 데이터를 바탕으로 몇 가지 특성을 추가했다. 추가 된 특성으로는 공모가대비 가격(현재가,시가,저가,고가)의 4가지 특성을 추가해 주었다. 
처음 상장한 주식의 가장 중요한 기준은 공모가이다. 공모가 자체의미가 그 회사의 가치를 어느정도 나타내는 것이고 이 가격을 기준으로 어느정도의 상승,하락의 지표가 될수 있기 때문이다.
## MinMax Scaler
주식 데이터는 각 특성마다. 가격의 scale이 다르기 때문에 너무 큰 차이는 모델의 학습에 영향을 줄수 있다. 그렇기 떄문에 0~1사이의 값으로 scaling을 진항하였다. 
